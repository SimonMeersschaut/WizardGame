...
class Menu:
    ...
    class Level:
        ...
        def __init__(self, index, last):
            ...
            if index+1 in Menu.BOSS_FIGHTS:
        ...
        def render(self):
            ...
            if self.x < x < self.x+Menu.Level.WIDTH and self.y < y < self.y+Menu.Level.WIDTH:
                Screen.draw_rect(self.x+Menu.Level.offset_x-2, self.y-2,
                                 Menu.Level.WIDTH+4, Menu.Level.WIDTH+4, color=(255, 255, 255))
            else:
                Screen.draw_rect(self.x+Menu.Level.offset_x-2, self.y-2,
                                 Menu.Level.WIDTH+4, Menu.Level.WIDTH+4, color=(40, 40, 40))
            Screen.draw_rect(self.x+Menu.Level.offset_x, self.y,
                             Menu.Level.WIDTH, Menu.Level.WIDTH, color=self.color)
            Screen.render_text(
                str(self.index+1), self.x+Menu.Level.offset_x+10, self.y, color=self.text_color)
    ...
    def render():
        ...
        if Screen.state == 'main_menu':
            ...
            Screen.renderIMG(
                'arrow_right.png', (960-Menu.X_VALUES[index]-75, index*(628-534)+509))
            Screen.renderIMG(
                'arrow_left.png', (960+Menu.X_VALUES[index]+75-50, index*(628-534)+509))
            if Screen.mouseDown:
                if index == 0:
