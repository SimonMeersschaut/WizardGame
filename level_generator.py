...
class Level:
  def __init__(self, data=None):
    self.inhoud = [[None for x in range(World.Config.world_size[0])] for y in range(World.Config.world_size[1])]
    if type(data) == list:
    ...
    for x in World.levels[World.current_level]:
      ...
      for y in x:
        if y != None:
          ...
          pygame.draw.rect(display, colour, (x_pos+offset, y_pos, World.Config.block_size[0], World.Config.block_size[1]))
...
if len(World.Config.colours) == 0:
  World.Config.colours = [(randint(0,255), randint(0,255), randint(0,255)) for color in World.Config.blocks]
...
while running:
  ...
  for event in pygame.event.get():
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
      x, y = event.pos
      buttons = [ function for x1, y1, width, height, function, _ in BUTTONS if (x1 < x < x1+width and y1 < y < y1+height) ]
      if any(buttons):
        buttons[0]()
      else:
        if event.button == 4:
          selected_block_index += 1
          selected_block_index = selected_block_index % (len(World.Config.blocks))
        elif event.button == 5:
          selected_block_index -= 1
        else:
          mouse_down = True
    ...
    if mouse_down:
      if button == 1:
        World.set_block(x-offset, y, World.Config.blocks[selected_block_index])
      elif button == 3:
        World.set_block(x-offset, y, None)
  if press:
    offset += speed
  for x1, y1, width, height, function, colour in BUTTONS:
