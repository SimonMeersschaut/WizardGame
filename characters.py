...
class Wizard:
    ...
    def renderMe(self):
        ...
        # RENDERING ALL IMGS
        self.screen.renderSurface(
            self.imgs[self.direction], (self.x-GLOBAL.variables["camera"].x, self.y))
        self.screen.blitRotate(
        ...
        if GLOBAL.variables["settings"].k_left in self.screen.keys:
            ...
            if not(self.world.get_block(pos) in self.world.standables):
                if GLOBAL.variables['settings'].k_sprint in self.screen.keys:
                    self.speed = min(
                        Wizard.SPRINT_SPEED, self.speed+GLOBAL.variables['screen'].frame_speed)
                else:
        ...
        if supported:
            ...
            if self.y_speed > 0:
                self.y_speed = 0
                self.y = int((self.y)/self.world.square_size) * \
                    self.world.square_size+(69-Wizard.FEED_Y)
        ...
        if self.floating and GLOBAL.variables['magic'].points > 0:
            ...
            self.y_speed = min(
                max(0, self.y_speed-(1.2*self.screen.frame_speed)), self.y_speed)
...
class Witch(Entity):
    ...
    def attack(self):
        ...
        if self.hp <= 0:
            ...
            if self.color == 0:
                ...
                Characters.createNew('darkmind_gray', self.x, self.y+130)
            else:
                self.target_y = self.start_y
                Characters.createNew('darkmind_red', self.x, self.y+130)
    ...
    def renderMe(self):
        ...
        #  Characters.wizard.x = max(self.x-1920, Characters.wizard.x)
        # self.check_collision()
