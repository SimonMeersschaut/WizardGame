...
class World:
                ...
                if not('book' in name):
                    if name in GLOBAL.variables['characters'].NAMES:
                        GLOBAL.variables['characters'].createNew(
                            name, x*World.square_size, y*World.square_size)
                        continue
                    try:
                        World.map[x][y] = name
                    except IndexError:
    ...
    def spawn_grass():
        ...
        for block, x, y in World.current_level:
            if block == 'ground.png' and World.get_block(x, y-World.square_size) == None:
                World.current_level.append(
                    ['grass.png', x, y-World.square_size])
                print('grass')
        ...
        for index, (obj, xpos, ypos) in enumerate(World.current_level):
            if type(obj) == str:
                if any([obj.split('.')[0] in BOTTOM or BOTTOM in obj.split('.')[0] for BOTTOM in BOTTOMS]) and (World.map[round(xpos/World.square_size)][round(ypos/World.square_size)-1] in World.grounds or World.map[round(xpos/World.square_size)][round(ypos/World.square_size)-1] in BOTTOMS):
                    ...
                    World.map[round(xpos/World.square_size)
                              ][round(ypos/World.square_size)] = new_name
    ...
    def load_next_level():
        World.level += 1
        GLOBAL.variables['screen'].time_speed = 1
        GLOBAL.variables['settings'].unlocked_levels = max(
            World.level, GLOBAL.variables['settings'].unlocked_levels)
    ...
    def render():
        ...
        width, height = ((World.time_limit-time()) /
                         World.level_time_limit*1500, 50)
        x1 = 200
        y1 = 10
        ...
        if World.state == 'completed':
            ...
            Screen.render_text('mission accomplished!'[
                               0:chars], 500, 300, color=(20, 20, 20))
        ...
        try:
            ...
            for text, posx, posy in list:
                ...
                Screen.render_text(text, posx-camera_x, posy,
    ...
    def delete_block(xpos, ypos):
        x_block, y_block = (int(xpos/World.square_size),
                            int(ypos/World.square_size))
        ...
        for index, (obj, xpos1, ypos1) in enumerate(World.current_level):
            x_block1, y_block1 = (
                int(xpos1/World.square_size), int(ypos1/World.square_size))
            if obj == block_name:
...
class Tutorial:
    TEXT = [
        [('use <WASD> to walk and jump with <SPACE>', 50, 200)],
        [('red darkminds will follow you', 295, 450), ('collect a book to perform spells',
                                                       1782, 403), ('perform a shield to reverse them', 3187, 446)]
    ]
#  @classmethod
